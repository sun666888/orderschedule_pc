        ...
        <div class="content-show">
            <div class="row list-search">
                ...
                <div class="col-md-3 search-field">
                    ...
                    <el-date-picker size="medium" type="date" v-model="order_start_time" placeholder="选择开始时间" value-format="yyyy-MM-dd">
                    </el-date-picker>
                </div>
                <div class="col-md-3 search-field">
                    <div style="left:0px" class="label">结束时间：</div>
                    <el-date-picker size="medium" type="date" v-model="order_end_time" placeholder="选择结束时间" value-format="yyyy-MM-dd">
                    </el-date-picker>
                </div>
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
    //   curingformSrv.searchRegionSolidifyOrder(vm.monthCompleteTime, vm.dayCompleteTime, vm.district).then(resp => {
  ...
  methods: {
    searchList() {
      this.loading = true
      curingformSrv.searchRegionSolidifyOrder(this.order_start_time, this.order_end_time, this.district).then(resp => {
        let data = resp.areaIndexExport
        // console.log(resp)
        this.loading = false
        if (data) {
    ...
    exportOrder() {
      ...
      let url = `/orderschedule/index.php/Home/Export/ExportAreaIndexSolidify?${Object.entries(searchContent).map(x => x.join('=')).join('&')}`
      this.download(url)
    },
    download(url) {
