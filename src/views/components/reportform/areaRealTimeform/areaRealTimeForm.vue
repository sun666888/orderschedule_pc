...
export default {
  ...
  methods: {
    searchList() {
      ...
      this.loading = true
      realformSrv.areaRealTimeExport(this.curDate).then(resp => {
        let data = resp.data
        this.loading = false
        // console.log(resp)
        // let orderTotal = {
    ...
    exportOrder() {
      ...
      let url = `/orderschedule/index.php/Home/V2/exportAreaRealTime?${Object.entries(searchContent).map(x => x.join('=')).join('&')}`
      this.download(url)
    },
    download(url) {
