...
export default {
  ...
  methods: {
    searchList() {
      ...
      this.loading = true
      realformSrv.realTimeDayExport(this.curDate).then(resp => {
        let data = resp.data
        this.loading = false
        // console.log(resp)
        // let orderTotal = {
    ...
    exportOrder() {
      ...
      let url = `/orderschedule/index.php/Home/V2/exportRealTimeDay?${Object.entries(searchContent).map(x => x.join('=')).join('&')}`
      this.download(url)
    },
    download(url) {
